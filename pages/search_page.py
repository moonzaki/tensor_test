...
class SearchPage(BasePage):
    ...
    def enter_on_search_form(self): #Нажатие enter, проверка наличия слова "тензор" в титуле страницы 
        ...
        time.sleep(5)
        try:
            assert "тензор" in self.browser.title, "Incorrect text search in title!"
        except (NoSuchElementException):
            return False
        return True
    ...
    def should_be_tensor_in_links(self): #Проверка наличия в первых 5 результатах ссылки на tensor.ru
        ...
        for listitem in lists:
            ...
            if t in href_a:
                elem += 1
        print(elem)        
        assert elem >= 5, f"Количество ссылок, содержащих 'tensor.ru': {elem}, ожидается: 5"
