...
class PicturePage(BasePage):
    ...
    def should_be_next_and_should_be_back(self): 
        ...
        current_picture = self.browser.find_element(*PicturePageLocators.PIC_STYLE).get_attribute("src")
        button_next.click()
        next_picture = self.browser.find_element(*PicturePageLocators.PIC_STYLE).get_attribute("src")
        assert current_picture != next_picture, "The picture has not changed"
        
    def should_be_back(self):
